...
const logLines = [
    ...
    { timestamp: '18:31:21.450', content: 'Cloning github.com/chenparnasa/isr-blog-nextjs-wordpress (Branch: main, Commit: de72693)' },
    ...
    { timestamp: '18:31:22.123', content: 'Running "appwrite build"' },
...
export function BentoSites() {
  ...
  return (
    <Link
      ...
      className="border-smooth group col-span-12 flex flex-col rounded-2xl border bg-white/2 p-2 transition-shadow duration-300 hover:shadow-[0px_0px_0px_4px_var(--color-offset)] focus:shadow-[0px_0px_0px_4px_var(--color-offset)] lg:col-span-7"
    >
      <div className="space-y-3 px-3 pt-2 pb-4">
        <div className="flex items-center gap-2">
          <img loading="lazy" src="/assets/images/icons/illustrated/dark/sites.png" alt="Sites icon" width={28} height={28} className="size-7" />
          <h3 className="font-aeonik-pro text-label text-primary">Sites</h3>
        </div>
        <p className="text-sub-body text-primary max-w-lg font-medium">
          <span className="text-secondary">Host and maintain</span> your website domains and frontend code. Integrated with all Clikkle products.
        </p>
      </div>
        ...
        <div className="border-smooth right-0 bottom-8 z-1 mx-auto flex w-full max-w-[450px] flex-col overflow-hidden rounded-[1.25rem] border bg-[#1D1D21] px-2 pb-2 md:absolute md:-right-24">
            <div className="flex h-10 w-full items-center gap-6 px-2 md:justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-secondary rounded-md bg-white/20 px-1.5 py-0.5 font-light leading-none">Building</span>
                </div>
                <div className="flex items-center gap-2">
                    <Loader2 className="h-3 w-3 animate-spin text-secondary" />
                    <span className="font-mono text-sm text-secondary">32s</span>
                </div>
            </div>
                ...
                <div className="w-[900px]">
                    {logLines.map(({ timestamp, content }, i) => (
                        <div key={i} className="flex flex-nowrap gap-3">
                            ...
                            <span className={cn('text-primary block flex-nowrap', {
                                'text-[#5382CB]': i === 0
                            })}>{content}</span>
                        </div>
                    ))}
                    <div className="flex flex-nowrap gap-3">
                        ...
                        <div className="relative">
                           <span className="text-primary block flex-nowrap max-w-[26ch] overflow-hidden whitespace-nowrap border-r-2 border-primary animate-[pulse_1s_infinite]">Installing dependencies...</span>
                        </div>
                    </div>
                </div>
            </div>
