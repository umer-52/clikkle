...
layout: article
title: Tokens
description: What are tokens and how to use them in Clikkle
---
Tokens are short-lived secrets created by an [Clikkle Server SDK](/docs/sdks#server) that can be exchanged for session by a [Client SDK](/docs/sdks#client) to log in users.
...
Many Clikkle authentication methods use a token-base flow to authenticate users. For token-based authentication methods, there are two high level steps to authenticate a user:

# Token login {% #token-login %}
You can find different usage of tokens in the Clikkle.
