...
layout: article
title: Collections
description: Organize your data with Clikkle Collections. Explore how to create and configure collections to store and structure your data effectively.
---
Clikkle uses collections as containers of documents. Each collection contains many documents identical in structure.
The terms collections and documents are used because the Clikkle JSON REST API resembles the API of a traditional NoSQL database, making it intuitive and user-friendly, even though Clikkle uses SQL under the hood.

That said, Clikkle is designed to support both SQL and NoSQL database adapters like MariaDB, MySQL, or MongoDB in future versions.

# Create collection {% #create-collection %}
You can create collections using the Clikkle Console, a [Server SDK](/docs/sdks#server), or using the [CLI](/docs/tooling/command-line/installation).
{% tabs %}
...
You can also create collections programmatically using a [Server SDK](/docs/sdks#server). Clikkle [Server SDKs](/docs/sdks#server) require an [API key](/docs/advanced/platform/api-keys).
...
To create your collection using the CLI, first use the `clikkle init collections` command to initialize your collection.

```sh
clikkle init collections
```

Then push your collection using the `clikkle push collections` command.

```sh
clikkle push collections
```

This will create your collection in the Console with all of your `clikkle.config.json` configurations.
...
Clikkle uses permissions to control data access.
...
By default, Clikkle doesn't grant permissions to any users when a new collection is created.
...
Attributes are used to define the structure of your documents and help the Clikkle's API validate your users' input.
...
To ensure the best performance, Clikkle recommends an index for every attribute queried.
