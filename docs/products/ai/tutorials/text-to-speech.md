...
description: Learn how to integrate Hugging Face into your Clikkle project for text to speech processing.
...
export default async ({ req, res, error }) => {
  const bucketId = process.env.CLIKKLE_BUCKET_ID ?? 'generated_speech';
...
stores the generated audio file in Clikkle Storage, and creates a row in the Clikkle database of the original text.
{% /section %}
...
Then, use the fileId to download the generated audio file from the Clikkle Storage.
{% /section %}
