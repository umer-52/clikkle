...
description: Learn how to integrate LangChain into your Clikkle project.
...
All other `POST` requests will trigger the indexing of the Clikkle table into the Pinecone index.
...
const clikkleRows = await clikkle.getAllRows(
  process.env.CLIKKLE_DATABASE_ID,
  process.env.CLIKKLE_TABLE_ID
);

const rows = clikkleRows.map(
  (row) =>
    new Row({
      metadata: { id: row.$id },
