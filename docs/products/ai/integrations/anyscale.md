...
description: Learn how to integrate Anyscale into your Clikkle project.
difficulty: intermediate
readtime: 15
---

The Anyscale API is a powerful tool for generating text using the leading open-source models. This tutorial will guide you through setting up the Anyscale API and integrating it into your Clikkle project.

You'll create a simple function that takes a text prompt and generates a completion using Mistral's Mixtral 8x7B model. Then, using Clikkle functions, you'll create a UI that allows users to input text and see the generated completion.
...
The form will allow users to submit text to the Clikkle function through a POST request. The Clikkle function will call the Anyscale API and return the response to the user.
{% /section %}

{% section #step-6 step=6 title="Handle POST request" %}
Add methods necessary to integrate with the Anyscale API.

Import `openai` and the Clikkle SDK at the top of the `main.js` file.

```js
import OpenAI from 'openai';
```

Add code to validate the body of the request and initialize the Clikkle SDK after the `GET` request handler from earlier:
...
![Testing the function](/clikkle/images/docs/ai/integrations/anyscale/demo.png)
{% /section %}
