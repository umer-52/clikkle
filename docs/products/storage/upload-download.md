...
description: Effortlessly upload and download files with Clikkle Storage. Learn how to handle file uploads, manage file versions, and ensure secure downloads in your applications.
---

You can upload and download files both programmatically using SDKs or through the Clikkle Console.

# Create file {% #create-file %}

After you create a bucket or have navigated to bucket details, you can access the **Files** tab so you can upload, view, delete and update files in the bucket using the Clikkle project's dashboard. You can also perform all those operations from Clikkle's client SDK, server SDKs, and REST APIs as long as you have the proper permission.
...
{% multicode %}
  ```client-web
  import { Client, Storage, ID } from "clikkle";
  ...
  const { InputFile } = require('node-clikkle/file');
  ...
  ```client-react-native
  import { Client, Storage, ID } from 'react-native-clikkle';
...
If you're using an Clikkle SDK, this is handled automatically.
If you're not using an SDK, you can [learn more about REST API file handling](/docs/apis/rest#files).

# InputFile {% #input-file %}
Every language and platform handles file inputs differently. This section rows the expected input type of each SDK. Where applicable, Clikkle provides an `InputFile` class to accept multiple file sources, like paths, buffers, or plain text.

# Client SDKs {% #client-sdks %}

{% tabs %}
{% tabsitem #web title="Web" %}
The Clikkle Web SDK expects a [File](https://developer.mozilla.org/en-US/docs/Web/API/File) object for file creation. This is most commonly associated with DOM file inputs.
...
The Clikkle Flutter SDK expects an `InputFile` class for file inputs.
...
The Clikkle Android SDK expects an `InputFile` class for file inputs.
...
The Clikkle Apple SDK expects an `InputFile` class for file inputs.
...
The Clikkle React Native SDK expects a file object with the following properties for file inputs:
...
In browser environments, you can use the `File` object directly. For Node.js environments, import the `InputFile` class from 'node-clikkle/file'.
...
The Clikkle PHP SDK expects an `InputFile` class for file inputs.
...
The Clikkle Python SDK expects an `InputFile` class for file inputs.
...
The Clikkle Ruby SDK expects an `InputFile` class for file inputs.
...
The Clikkle Deno SDK expects an `InputFile` class for file inputs.
...
The Clikkle Dart SDK expects an `InputFile` class for file inputs.
...
The Clikkle Kotlin SDK expects an `InputFile` class for file inputs.
...
The Clikkle Swift SDK expects an `InputFile` class for file inputs.
...
The Clikkle .NET SDK expects an `InputFile` class for file inputs.
...
```
```client-react-native
import { Client, Storage } from 'react-native-clikkle';
...
```
```client-react-native
import { Client, Storage, ImageGravity } from 'react-native-clikkle';
