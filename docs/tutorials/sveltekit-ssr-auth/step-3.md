...
Notice that `createClikkleClient` returns **a new instance** of the Clikkle Client.
...
import { CLIKKLE_KEY } from '$env/static/private';
import { PUBLIC_CLIKKLE_ENDPOINT, PUBLIC_CLIKKLE_PROJECT } from '$env/static/public';
...
export function createAdminClient() {
    const client = new Client()
        ...
        .setKey(CLIKKLE_KEY); // Set the Clikkle API key!
...
`CLIKKLE_KEY`, `PUBLIC_CLIKKLE_ENDPOINT` and `PUBLIC_CLIKKLE_PROJECT_ID` are environment variables that are exported in your project's [.env file](https://kit.svelte.dev/docs/modules#$env-dynamic-public).
...
The `CLIKKLE_KEY` is an Clikkle API key with the necessary permissions to read and write accounts and sessions.
