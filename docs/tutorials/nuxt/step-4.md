...
description: Add authentication to your Nuxt application using Clikkle Web SDK
step: 4
---
...
We are using Clikkle as a backend to handle the user details, so we need to connect to Clikkle by importing the configurations from step 3.
...
import { ID } from "clikkle";
import { ref } from "vue";
import { account } from "../clikkle";
