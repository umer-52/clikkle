...
layout: article
title: Start with Go
description: Integrating Clikkle with your Go backend application is a quick and simple process. Get your backend up and running with our step-by-step guide.
difficulty: beginner
readtime: 5
back: /docs/quick-starts
---
Learn how to set up your first Go project powered by Clikkle.
...
mkdir my-app
cd my-app
go mod init go-clikkle/main
```

{% /section %}
{% section #step-3 step=3 title="Install Clikkle" %}

Install the Go Clikkle SDK.

```sh
go get github.com/clikkle/sdk-for-go
```
...
Create a new file called `app.go`, initialize a function, and initialize the Clikkle Client. Replace `<PROJECT_ID>` with your project ID and `<YOUR_API_KEY>` with your API key. Import the Clikkle dependencies for clikkle, client, databases, and models.
...
import (
	...
	"github.com/clikkle/sdk-for-go/tablesDB"
	"github.com/clikkle/sdk-for-go/models"
	"github.com/clikkle/sdk-for-go/query"
)

var (
	clikkleClient    client.Client
	todoDatabase      *models.Database
	todoTable    *models.Table
	clikkleDatabases *tablesDB.TablesDB
)

func main() {
	clikkleClient = clikkle.NewClient(
		clikkle.WithProject("<PROJECT_KEY>"),
...
Once the Clikkle Client is initialized, create a function to configure a todo table. Import the id Clikkle dependency by adding `"github.com/clikkle/sdk-for-go/id"` to the imported dependencies list.

```go
func prepareDatabase() {
	tablesDB = clikkle.NewTablesDB(clikkleClient)
