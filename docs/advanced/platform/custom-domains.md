...
description: Customize your Clikkle platform with custom domains. Learn how to set up and configure custom domains to provide a branded experience for your users.
---

Clikkle custom domains allows you to use your own domain as your Clikkle API endpoint. 
...
When accessing Clikkle from a 3rd party domain, like `cloud.clikkle.io` or `example.com`, 
some browsers will treat our secure cookies as 3rd-party cookies and block them, 
as a fallback Clikkle will store your users' sessions on the browser localStorage.

Using localStorage is very convenient to help you get started quickly with Clikkle, but it is not the best practice for your users' security. 
...
# Clikkle API endpoint {% #endpoint %}
To prevent your browser from blocking your cookies, your Clikkle API endpoint should be set to under same domain of your web app's domain.
When accessing Clikkle from the same domain as the one your app uses, 
Clikkle cookies will no longer be treated as 3rd-party cookies by any browser and will store your users' sessions securely.

For example, if your app runs on [my-app.com](https://my-app.com), 
you can set the subdomain [clikkle.my-app.com](https://clikkle.my-app.com) to access the Clikkle API. 
This will allow browsers to respect the Clikkle sessions cookies as they are set on the same domain as your app.

# Add a custom domain {% #domain %}

1. Go to the Clikkle Console and navigate to your project.
...
With these steps, your Clikkle project will accept API requests from your custom domain.
...
DNS changes might take up to 48 hours to propagate worldwide. This means that it might take up to two days for your new domain to become accessible using Clikkle. For debugging, you can try using [this online tool](https://dnschecker.org/) to check your DNS propagation status.

In addition to the DNS setup, you might also want to update the "Allowed Domains" section in your Clikkle project settings. By default, Clikkle only allows API calls from localhost, clikkle.io, and your project's default custom domains. You can add your custom domain to this list to ensure that API requests from your domain are accepted.
