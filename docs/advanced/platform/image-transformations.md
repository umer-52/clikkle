...
description: Learn how to transform images using Clikkle's storage API.
---
...
Clikkle enables the transformation of images before retrieval using the [getFilePreview](/docs/references/cloud/client-web/storage#getFilePreview) endpoint. This functionality supports resizing images by width and height, adjusting quality, and applying filters such as opacity, border colour, border radius, and more.

### Origin Image

An "origin image" represents the original, unmodified image file in Clikkle Storage. Each origin image serves as the base for unlimited transformations, allowing the creation of multiple variants without incurring additional origin image charges.

How it works:

1. Upload an image to Clikkle Storage
