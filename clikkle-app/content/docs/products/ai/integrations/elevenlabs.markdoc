...
description: Learn how to integrate ElevenLabs into your Clikkle project.
...
Integrating ElevenLabs into your Clikkle project is simple. This tutorial will guide you through the process of setting up the ElevenLabs API and integrating it into your Clikkle project.
...
1. In the **Variables** step, add the `ELEVENLABS_API_KEY`, generate it [here](https://elevenlabs.io/). For the `CLIKKLE_API_KEY`, tick the box to **Generate API key on completion**.
...
Install the `undici` package to make requests to the ElevenLabs API and `node-clikkle` package to upload the generated audio files to Clikkle Storage.
...
A check is also included to ensure that the `ELEVENLABS_API_KEY`, `CLIKKLE_API_KEY` and `CLIKKLE_BUCKET_ID` environment variables is set.
{% /section %}
...
All of this together will render a form that will submit your text to the Clikkle function through a POST request which you'll create next. The Clikkle function will call ElevenLabs's API, upload the audio to Clikkle Storage and return the URL, which will be displayed on your page.
{% /section %}

{% section #step-6 step=6 title="Handle POST Request" %}
Add methods necessary to integrate with the ElevenLabs API:

Import `fetch`, and the required features from the Clikkle Node.js SDK at the top of the `main.js` file
...
Next add code to validate the body of the request and initialize the Clikkle SDK:

```js
const client = new Client()
  .setEndpoint(process.env.CLIKKLE_ENDPOINT ?? "https://<REGION>.cloud.clikkle.io/v1")
...
```js
const url = `${process.env.CLIKKLE_ENDPOINT}/storage/buckets/${process.env.CLIKKLE_BUCKET_ID}/files/${file.$id}/view?project=${process.env.CLIKKLE_FUNCTION_PROJECT_ID}`;

return res.json({ ok: true, response: url });
```

This should finish up the function, Deploy it to Clikkle by pushing to the git repository created earlier.
{% /section %}
...
![Testing the function](/clikkle/images/docs/ai/integrations/elevenlabs/demo.png)
{% /section %}
