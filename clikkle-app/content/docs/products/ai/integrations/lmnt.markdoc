...
description: Learn how to integrate LMNT into your Clikkle project.
...
Integrating LMNT into your Clikkle project is simple. This tutorial will guide you through setting up the LMNT API and incorporating it into your Clikkle project.
...
Head to the [Clikkle Console](https://cloud.clikkle.io/console), click on **Functions** in the left sidebar and click the **Create Function** button.
...
5. In the **Variables** step, add `CLIKKLE_BUCKET_ID`, `LMNT_API_KEY`. Generate your LMNT Key [here](https://app.lmnt.com/account). For the `CLIKKLE_API_KEY`, tick the box to **Generate API key on completion**.
...
Install the `lmnt-node` package to make requests to the LMNT API and `node-clikkle` package to upload the generated audio files to Clikkle Storage.

```bash
npm install lmnt-node node-clikkle
```
{% /section %}
...
A check is also included to ensure that the `LMNT_API_KEY`, `CLIKKLE_API_KEY` and `CLIKKLE_BUCKET_ID` environment variables are set.
{% /section %}
...
<head>
  ...
  <link rel="stylesheet" href="https://unpkg.com/@clikkle.io/pink" />
  <link rel="stylesheet" href="https://unpkg.com/@clikkle.io/pink-icons" />
</head>
```
...
All of this together will render a form that will submit your text to the Clikkle Function through a `POST` request. The Clikkle function will then call LMNT's API, upload the audio to Clikkle Storage and return the URL, which will be displayed on your page.
{% /section %}

{% section #step-6 step=6 title="Handle POST Request" %}
Next, you'll add the methods necessary to integrate with the LMNT API.

Import the `Speech` class from `lmnt-node`, and the required features from the Clikkle Node.js SDK at the top of the `main.js` file.

```js
import { Client, Storage, ID, Permission, Role } from "node-clikkle";
import { InputFile } from "node-clikkle/file";
import Speech from 'lmnt-node';
```

Next, add code to validate the body of the request and initialize the Clikkle SDK also within `main.js` following the previously added GET handler:

```js
const endpoint = process.env.CLIKKLE_ENDPOINT ?? "https://<REGION>.cloud.clikkle.io/v1";
...
{% section #step-7 step=7 title="Store Audio in Clikkle Storage" %}
Store the audio file in Clikkle Storage for easy retrieval later:

```js
const storage = new Storage(client);

const file = await storage.createFile({
  bucketId: process.env.CLIKKLE_BUCKET_ID,
  fileId: ID.unique(),
...
To show it to the user, parse the download URL from Clikkle and return it in the response:

```js
const url = `${endpoint}/storage/buckets/${process.env.CLIKKLE_BUCKET_ID}/files/${file.$id}/view?project=${process.env.CLIKKLE_FUNCTION_PROJECT_ID}`;

return res.json({ ok: true, response: url });
```

This should finish up the Function. Deploy it to Clikkle by pushing it to the git repository created earlier.
{% /section %}
...
![Testing the function](/clikkle/images/docs/ai/integrations/lmnt/demo.png)
{% /section %}
