...
The Clikkle CLI provides a simple way to generate types based on your Clikkle database schema. This feature is particularly useful for developers who want to ensure type safety in their applications by generating type definitions that match their database collections and attributes.

To generate types, the CLI reads the database schema from your project's `clikkle.json` file and generates type definitions for each collection.

## Generating types

First, ensure you have the [Clikkle CLI](/docs/tooling/command-line/installation#getting-started) installed and your project is [initialised](/docs/tooling/command-line/installation#initialization). Then, run the following command in your terminal to pull collections from your Clikkle project:

```bash
clikkle pull collections
```
...
Suppose you want to generate types for a collection with data on books with the following schema from your `clikkle.json` file:
...
```js
/**
 * @typedef {import('clikkle').Models.Document} Document
 */
