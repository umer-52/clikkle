...
On commit, Clikkle replays all staged logs in order inside a real database transaction. Staged operations see earlier staged changes (read your own writes). If any affected row changed outside your transaction, the commit fails with a conflict.
...
```
```client-react-native
import { Client, TablesDB } from 'react-native-clikkle';
...
When you pass `transactionId`, Clikkle writes the operation to an internal staging area. The target table is not modified until you commit the transaction.
...
On commit, Clikkle verifies that rows affected by your transaction haven’t changed externally since they were staged. If a conflicting change is detected, the commit fails with a conflict error. Resolve the conflict (for example, refetch and re-stage) and try again.
