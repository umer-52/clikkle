...
description: Understand Clikkle platform response codes for effective error handling. Learn how to interpret and handle response codes to enhance your application's reliability.
---

Clikkle uses conventional HTTP response codes to indicate the success or failure of an API request.

- Codes in the `2xx` range indicate success.
- Codes in the `4xx` range indicate an error caused by invalid request, usually caused by user error.
- Codes in the `5xx` range indicate an error with Clikkle, please check Docker container logs.
...
| 500  | Internal Server Error  | Something is broken. Contact our [team](/support), or raise a [GitHub issue](https://github.com/clikkle/clikkle/issues/new).                                                                  |
...
| 503  | Service Unavailable    | The Clikkle servers are up but overloaded with requests. Try again later.                                                                                                                      |
| 504  | Gateway timeout        | The Clikkle servers are up, but the request couldn't be serviced due to some failure within the stack. Try again later.                                                                       |

# Error messages {% #error-messages %}

When the Clikkle APIs return error messages, it does so in JSON format. For example, an error might look like this:
...
Clikkle also passes convenient error types in addition to the HTTP response codes to help you get more fine-grained control over what went wrong and allowing you to display relevant error messages in your applications. Error types are convenient to identify the type of error that occurred.

For example, a `400` HTTP response code could indicate a Bad Request due to a variety of reasons, and error types can help you pinpoint the exact `400` error. Clikkle currently supports the following error types:

## Platform errors {% #platform-errors %}
Platform errors are not related to an individual product, but can occur across the Clikkle platform.
...
| 400  | project_unknown | The project ID is either missing or not valid. Please check the value of the `X-Clikkle-Project` header to ensure the correct project ID is being used. |
...
| 401  | project_key_expired | The project key has expired. Please generate a new key using the Clikkle console. |
...
| 403  | general_unknown_origin | The request originated from an unknown origin. If you trust this domain, please list it as a trusted platform in the Clikkle console. |
...
| 404  | project_not_found | Project with the requested ID could not be found. Please check the value of the `X-Clikkle-Project` header to ensure the correct project ID is being used. |
...
| 405  | general_not_implemented | This method was not fully implemented yet. If you believe this is a mistake, please upgrade your Clikkle server version. |
...
| 412  | project_provider_disabled | The chosen OAuth provider is disabled. You can enable the OAuth provider using the Clikkle console. |
...
| 500  | router_domain_not_configured | `_APP_DOMAIN`, `_APP_DOMAIN_TARGET`, and `_APP_DOMAIN_FUNCTIONS` environment variables have not been configured. Please configure the domain environment variables before accessing the Clikkle Console via any IP address or hostname other than localhost. This value could be an IP like 203.0.113.0 or a hostname like example.com. |
| 501  | general_usage_disabled | Usage stats is not configured. Please check the value of the `_APP_USAGE_STATS` environment variable of your Clikkle server. |
...
| 503  | general_service_disabled | The requested service is disabled. You can enable the service from the Clikkle console. |
| 503  | general_smtp_disabled | SMTP is disabled on your Clikkle instance. You can learn more about setting up SMTP in our docs. |
| 503  | general_phone_disabled | Phone provider is not configured. Please check the `_APP_SMS_PROVIDER` environment variable of your Clikkle server. |

## Authentication errors {% #authentication-errors %}
Errors found when using Clikkle Authentication.
| Code | Type | Description |
| ---- | ---- | ----------- |
...
| 401  | user_jwt_invalid | The JWT token is invalid. Please check the value of the `X-Clikkle-JWT` header to ensure the correct token is being used. |
| 401  | user_blocked | The current user has been blocked. You can unblock the user by making a request to the User API's "Update User Status" endpoint or in the Clikkle Console's Auth section. |
...
| 501  | user_count_exceeded | The current project has exceeded the maximum number of users. Please check your user limit in the Clikkle console. |
| 501  | user_auth_method_unsupported | The requested authentication method is either disabled or unsupported. Please check the supported authentication methods in the Clikkle console. |

## Databases errors {% #databases-errors %}
Errors found when using Clikkle Databases.
| Code | Type | Description |
| ---- | ---- | ----------- |
...
Errors found when using Clikkle Storage.
| Code | Type | Description |
| ---- | ---- | ----------- |
...
| 400  | storage_invalid_clikkle_id | The value for `x-clikkle-id` header is invalid. Please check the value of the `x-clikkle-id` header is a valid id and not `unique()`. |
...
Errors found when using Clikkle Functions.
| Code | Type | Description |
| ---- | ---- | ----------- |
...
| 404  | function_runtime_unsupported | Entrypoint for your Clikkle Function is missing. Please specify it when making deployment or update the entrypoint under your function's "Settings" > "Configuration" > "Entrypoint". |
...
Errors when using Clikkle Migrations.
| Code | Type | Description |
| ---- | ---- | ----------- |
...
| 409  | migration_in_progress | Migration is already in progress. You can check the status of the migration in your Clikkle Console's "Settings" > "Migrations". |


## Avatars errors {% #avatars-errors %}
Errors from the Clikkle Avatars API.
| Code | Type | Description |
| ---- | ---- | ----------- |
...
Errors from the Clikkle GraphQL API.
| Code | Type | Description |
| ---- | ---- | ----------- |
