...
description: Deploy Clikkle on Google Cloud Platform using Compute Engine. Learn how to set up a production-ready Clikkle instance on GCP.
---

Deploy Clikkle on Google Cloud Platform using Compute Engine virtual machines.

# Compute Engine deployment {% #compute-engine %}

Google Cloud Compute Engine provides virtual machines where you can deploy Clikkle with full control over the infrastructure. Create a VM instance with at least 2 vCPU and 4 GB RAM, configure firewall rules to allow HTTP/HTTPS traffic, then follow the [general installation guide](/docs/advanced/self-hosting/installation) for Docker setup.
