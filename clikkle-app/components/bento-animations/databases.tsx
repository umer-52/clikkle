...
const collections = [
  { Category: "Sweaters", Department: "Sweaters", barcode: "Sweaters", ID: "3397fecdedb1" },
  { Category: "Pants", Department: "Pants", barcode: "Pants", ID: "2224gabjger4" },
  { Category: "T-shirts", Department: "T-shirts", barcode: "T-shirts", ID: "5689fdoerre2" },
  { Category: "Shoes", Department: "Shoes", barcode: "Shoes", ID: "5689fdoerre2" },
  { Category: "Accessories", Department: "Accessories", barcode: "Accessories", ID: "5689fdoerre2" },
];

const collectionKeys = Object.keys(collections[0]!);

const products = [
  { ID: 3397, name: "Turtlenecks", barcode: "TRTL" },
  { ID: 2224, name: "Pullovers", barcode: "PLOV" },
  { ID: 5689, name: "Zip-up", barcode: "ZPUP" },
];

const productKeys = Object.keys(products[0]!);

const overlaySpring = { type: "spring" as const, bounce: 0.3, duration: 0.35 };

function IdCellPill({ item }: { item: string }) {
  return (
    <span className="text-secondary border-smooth bg-greyscale-900 rounded-md border p-1.5">
      <svg
        width="17"
        height="17"
        viewBox="0 0 17 17"
    ...
    >
      <div className="space-y-3 px-3 pt-2 pb-4">
        ...
        <p className="text-sub-body text-primary max-w-lg font-medium">
          ...
          <span className="text-secondary">backed by your favorite technologies.</span>
        </p>
      </div>
      <div className="relative mt-auto mb-0 flex h-85 max-h-85 flex-1 flex-col overflow-hidden rounded-xl bg-black/24 px-8">
            ...
            <table className="table w-full p-2.5">
              <thead>
                <tr className="bg-greyscale-900 border-smooth text-primary text-eyebrow w-full border-b font-normal tracking-tight">
                  {collectionKeys.map((heading) => (
                    <th
                      key={heading}
                      className="p-2 text-left first-of-type:rounded-tl-xl last-of-type:rounded-tr-xl"
                    >
              ...
              <tbody className="text-eyebrow divide-smooth divide-y">
                {collections.map((collection, c) => {
                  ...
                  return (
                    <tr
                      key={collection.Category + collection.ID}
                      className={cn("bg-[#1D1D21] transition-colors", {
                        "bg-white/2": isSweaters && hovered,
                      })}
                    >
                      {vals.map((item, i) => {
                        const isLastProduct = c === collections.length - 1;
                        const isLastItem = i === vals.length - 1;
                        return (
                          <td
                            key={i}
                            className={cn("text-primary p-2.5 text-left", {
                              "rounded-bl-xl": isLastProduct && i === 0,
                              "rounded-br-xl": isLastProduct && isLastItem,
                            })}
                          >
                            {isLastItem ? <IdCellPill item={String(item)} /> : item}
                          </td>
                        );
                      })}
                    </tr>
        ...
        <div className="mt-0 mb-auto flex h-full w-full flex-col gap-8">
          <motion.div
            className="border-smooth absolute right-8 bottom-8 flex aspect-[4/2] flex-col rounded-2xl border bg-[#232325]/90 shadow-[4px_8px_20px_rgba(0,0,0,0.2)] backdrop-blur-md md:min-w-[275px]"
            initial={false}
            animate={hovered ? { x: -5, y: -5 } : { x: 12, y: 12 }}
              ...
              <table className="table w-full p-2.5">
                ...
                <tbody className="text-eyebrow divide-smooth divide-y">
                  {products.map((product, p) => {
                    ...
                    return (
                      <tr key={product.ID} className="bg-[#1D1D21]">
                        {vals.map((item, i) => {
