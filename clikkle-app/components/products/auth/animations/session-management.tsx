...
export function SessionManagement() {
    const sessionData = [
        { icon: '/clikkle/images/products/auth/chrome.svg', location: 'New York, NY', device: 'Macbook Pro' },
        { icon: '/clikkle/images/products/auth/safari.svg', location: 'London, UK', device: 'iPhone 12' },
        { icon: '/clikkle/images/products/auth/firefox.svg', location: 'Sydney, Australia', device: 'iPad Air' }
    ];
                                    ...
                                    {sessionData.map((data, i) => (
                                        <div key={i} className="flex flex-1 items-center justify-between px-6 py-3 text-xs text-[#656569]">
                                            <div className="flex h-8 basis-2/3 items-center gap-2 text-left">
                                                <img src={data.icon} alt={data.device} className="size-6 opacity-70 grayscale" loading="lazy" />
